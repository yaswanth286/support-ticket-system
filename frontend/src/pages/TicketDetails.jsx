...
export default function TicketDetails() {
  ...
  async function handleFieldUpdate(field, value) {
    ...
    try {
      ...
      setUpdateError(err.response?.data?.message || 'Unable to update ticket.');
    } finally {
      setUpdating(false);
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setUpdateError(err.response?.data?.message || 'Unable to delete ticket.');
    }
  }
      ...
      <div className="ticket-detail-grid">
        <div>
          ...
          <div className="card">
            ...
            <CommentForm onSubmit={handlePostComment} submitting={commentSubmitting} />
          </div>
        </div>
        ...
        <div>
          <div className="card" style={{ marginBottom: 16 }}>
            ...
            <div className="detail-row"><span>Customer</span><span>{ticket.customer_name}</span></div>
            <div className="detail-row"><span>Email</span><span>{ticket.customer_email}</span></div>
            <div className="detail-row"><span>Assigned Agent</span><span>{ticket.assigned_agent_name || 'Unassigned'}</span></div>
            <div className="detail-row"><span>Created</span><span>{new Date(ticket.created_at).toLocaleString()}</span></div>
            <div className="detail-row"><span>Updated</span><span>{new Date(ticket.updated_at).toLocaleString()}</span></div>
          </div>
          ...
          {isAgent && (
            <div className="card">
              ...
              <div className="form-group">
                <label className="form-label" htmlFor="assigned_to">Assign to agent</label>
                <select
                  id="assigned_to"
                  className="form-select"
